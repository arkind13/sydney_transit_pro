...
if "journey" not in st.session_state: st.session_state.journey = initialise_state()
if "freeze_time" not in st.session_state: st.session_state.freeze_time = None

def get_now():  # Always Sydney TZ
    import pytz
...
with st.sidebar:
    ...
    presets = {"🏠 Mount Druitt": (-33.7690, 150.8192),
               ...
               "🏠 Redern": (-33.8923, 151.1988),
               "🏁 Central": (-33.8828, 151.2067)}

    selected_preset = st.selectbox("Current Simulated Location:", list(presets.keys()))
    sim_lat, sim_lng = presets[selected_preset]
    st.divider()
    api_status = ping_api()
    if api_status["success"]: st.success("🔵 TfNSW API: Connected")
    else: st.error("🔴 TfNSW API: Connection Failed")
    google_status = ping_google()
    if google_status["success"]: st.success("🟢 Google Geo: Connected")
    else: st.warning(f"🟡 Google Geo: {google_status['message']}")

    from search_history import get_last_geocode_source as lgcs
    source_labels = {"station_dictionary": "📚 Station Lookup",
    ...
    st.caption(f"📍 Origin: {source_labels.get(src_o, src_o)}\n\n🎯 Destination: {source_labels.get(src_d, src_d)}")
...
if st.session_state.journey.status == "IDLE":
    ...
    if recent:
        ...
        selected_history = st.selectbox("📋 Recent Searches", options=history_labels, index=0, label_visibility="collapsed")
    ...
    with col1:
        origin_raw = st.text_input("Origin", value=st.session_state.get("prefill_origin", "Mount Druitt Train Station"))
    with col2:
        destination_raw = st.text_input("Destination", value=st.session_state.get("prefill_destination", "Alexandria"))

    if st.button("🔎 Find Best Options", type="primary"):
        search_origin = origin_raw
        if "station" not in origin_raw.lower(): search_origin = f"{origin_raw} Station, Sydney"
        search_destination = destination_raw

        lat_o, lng_o, addr_o = get_coordinates(search_origin); source_o = lgcs()
        lat_d, lng_d, addr_d = get_coordinates(search_destination); source_d = lgcs()
        ...
        from geocoder import clean_address as ca
        name_o = _get_stn(search_origin) or search_origin
        name_d = ca(search_destination)

        if lat_o is not None and lat_d is not None:
            # Freeze clock at time of request for consistent plan
            st.session_state.freeze_time = get_now()
            ...
            else: st.warning("No routes found. Please check stop names.")
        else:
            if lat_o is None: st.error(f"❌ Could not locate origin: '{origin_raw}'. Try a station name.")
            if lat_d is None: st.error(f"❌ Could not locate destination: '{destination_raw}'. Try removing unit numbers.")

    if "journey_options" in st.session_state:
        st.subheader(f"Results from {st.session_state.search_origin}")
        o = st.session_state.get("search_origin", ""); d = st.session_state.get("search_destination", "")
        fav = is_favourite(o, d); fav_label = "⭐ Saved" if fav else "☆ Save"
        if st.button(fav_label, key="fav_current"):
            toggle_favourite(o, d); st.rerun()
        for i, opt in enumerate(st.session_state.journey_options):
            with st.container(border=True):
                ...
                if c4.button("Start Journey", key=f"start_{i}"):
                    ...
                    st.session_state.journey = start_journey(st.session_state.journey,
                                                             origin_gps=start_pos,
                                                             destination=st.session_state.search_destination,
                                                             active_route={"leg_bundles": opt["leg_bundles"], "total_minutes": opt["total_minutes"]})
                    st.rerun()
    favs = get_favourites()
    if favs:
        with st.expander("⭐ Saved Favourites", expanded=False):
            for fav in favs:
                ...
                if fc2.button("❌", key=f"unfav_{fav['origin']}_{fav['destination']}"):
                    toggle_favourite(fav["origin"], fav["destination"]); st.rerun()

elif st.session_state.journey.status in ["ACTIVE", "ALERTED"]:
    st.title(f"🚆 Tracking to {st.session_state.journey.destination}")
    current_pos = (sim_lat, sim_lng) if test_mode else get_live_gps(mode="browser")
    if "last_alt_check" not in st.session_state: st.session_state.last_alt_check = get_now()
    if "pause_alerts" not in st.session_state: st.session_state.pause_alerts = False
    time_since_check = (get_now() - st.session_state.last_alt_check).total_seconds()

    col_track, col_check, col_pause = st.columns([2.5, 1, 1])
    with col_check: manual_check = st.button("🔍 Check Now", use_container_width=True)
    with col_pause:
        if st.session_state.pause_alerts:
            if st.button("▶️ Resume Alerts", use_container_width=True, type="primary"):
                st.session_state.pause_alerts = False; st.rerun()
        else:
            if st.button("⏸️ Pause Alerts", use_container_width=True): st.session_state.pause_alerts = True; st.rerun()
    ...
    if not st.session_state.pause_alerts:
        if time_since_check >= 90 or manual_check:
            ...
            if alt_result:
                ...
                st.session_state.journey.watchdog_result = {}; st.rerun()
    if st.session_state.pause_alerts:
        ...
        if "last_alt_result" in st.session_state:
            ...
            if result is None:
                ...
                st.markdown(f"| | Current Route | New Route |\n|---|---|---|\n| ⏱️ Travel Time | {current_time} min | {new_time} min |\n| 🔄 Change Point | — | **{result['change_point']}** |\n| 💡 Saves | — | **{savings} min** |")
    ...
    if st.session_state.journey.status == "ALERTED" and st.session_state.journey.watchdog_result and "new_route" in st.session_state.journey.watchdog_result:
        ...
        if col_acc.button("✅ Accept New Route"):
            ...
            st.session_state.last_alt_result = None
            st.success("Journey Updated!"); st.rerun()
        if col_rej.button("❌ Dismiss"):
            st.session_state.journey.status = "ACTIVE"
            st.session_state.last_alt_result = None; st.rerun()
    if st.button("← Back to Search"):
        st.session_state.journey = initialise_state(); st.rerun()
        ...
        with st.expander(f"{icon} {bundle['action']}: {bundle['origin']} ⮕ {bundle['destination']}", expanded=True):
            if bundle["stops"]:
                for stop in bundle["stops"]:
                    ...
                    if is_passed: st.markdown(f":green[✅ {stop['name']} (Passed)]")
                    else: st.markdown(f"◦ {stop['name']} ({stop['planned_arrival']})")
                    global_stop_idx += 1
            else: st.markdown(f"_{bundle['action']} for {bundle['duration_min']} mins_")
    st.divider()
    ...
    if st.button("🛑 End Journey", use_container_width=True, type="primary"):
        st.session_state.journey = complete_journey(st.session_state.journey); st.rerun()
